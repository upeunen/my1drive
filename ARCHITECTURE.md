...
*   **Отдельные менеджеры (Декомпозиция)**:
    *   `OtgConnectionManager`: Управляет подключением SAF, разрешениями, поллингом (проверкой физического подключения OTG), изолированным автопоиском архивов строго в рамках активного тома `volumeId`, пакетной регистрацией выбранных пользователем архивов в Room (`selectArchivesFromDiscovery`), поддержанием стабильного множества `connectedArchiveUuids` подключенного тома, диалогами монтирования/выбора архивов и безопасным извлечением с гарантированной отменой и ожиданием завершения всех фоновых I/O операций.
    ...
    *   `ArchiveSyncHelper`: Синглтон (`getInstance`), выполняющий фоновую работу: двухфазная прогрессивная синхронизация архивов (Фаза 1: мгновенный импорт из `my1drive_db.json` и немедленный `refresh` галереи за ~0.5 сек; Фаза 2: фоновое потоковое сканирование `fastListFiles`, сверка «Диск — источник истины», очистка удаленных файлов, пакетная синхронизация Room через карту в оперативной памяти без поштучных SQL-запросов), синхронизация нескольких архивов на одном носителе с явным `targetArchiveUuid`, генерация миниатюр, отслеживание прогресса и безопасная остановка.
    ...
    *   **`GalleryDisplayManager`**: Отвечает за логику сортировки (`deviceSortMode`, `archiveSortMode`), фильтрацию по активному чипу или просмотр всех архивов (`archiveFilterUuid`), быструю однопроходную группировку элементов архива по годам и месяцам на корутинах `Dispatchers.Default` (без тяжелогласных `Calendar.getInstance()`), мемоизацию расчета Bento-блоков (`bentoCache`) и формирование горячих потоков `archiveYearGroups` и `archivedGroupedItems` (`SharingStarted.Eagerly`).
    *   `ThumbnailSyncManager`: Управляет корутинами и состояниями ручной и бережной фоновой («тихой») синхронизации миниатюр. Автоматически обходит все подключенные архивы (`connectedArchiveUuids`) с приоритетом на активный просматриваемый архив, мягко приостанавливается при скролле пользователя (`isScrollingFlow`) и реактивно перезапускается при переключении чипов архивов.
