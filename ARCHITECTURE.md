...
*   **UI Components (Jetpack Compose)**: `GalleryScreenContent`, `FullscreenPreview`, `InfoDialog`, `DisconnectedOtgInfoDialog`, `PhotosGridTab`, `ArchiveRoute`, `ArchivesAndFoldersDialog` (единый полноэкранный диалог управления архивами, поиска папок с фото с настраиваемой глубиной и вызова проводника SAF в стиле Мастера), `SettingsTab` (компоненты `SettingsDashboardTiles`, `FilesAndSyncSection`, `ArchivesSettingsSection`, `LanguageSettingsSection`, `AdvancedSettingsSection`).
...
*   **Отдельные менеджеры (Декомпозиция)**:
    ...
    *   `ArchiveSyncHelper`: Синглтон (`getInstance`), выполняющий фоновую работу: потоковое рекурсивное сканирование всех подпапок архива (`fastListFiles` на прямых запросах `ContentResolver.query` с очередью `ArrayDeque`, без пропуска папок с `.nomedia`), синхронизация «Диск — источник истины» с Room и `my1drive_db.json`, фоновая генерация кэшированных миниатюр с пакетным сохранением в БД через `insertAll`, отслеживание прогресса синхронизации через `StateFlow` / `SharedFlow` и безопасная остановка с гарантированным сбросом флагов отмены.
