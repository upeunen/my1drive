...
### Версия v1.1.2 (Архитектурный рефакторинг UI и состояний OTG)
* **Изменения**: UI-декомпозиция, оптимизация поллинга OTG, устранение мерцаний интерфейса и SOLID рефакторинг менеджера подключений.
* **Архитектурные детали**:
  - Баннеры состояний (Подключение, Диск не распознан, Диск отключен) вынесены из `GalleryScreenContent` в отдельный модуль `GalleryBanners.kt` (в соответствии с правилом ограничения строк в файле).
  - Произведен SOLID-рефакторинг `OtgConnectionManager`: он больше не содержит состояний UI-диалогов. Теперь он выполняет исключительно бизнес-логику опроса физического подключения, а управление UI делегируется в `GalleryViewModel` через callback-интерфейс в конструкторе.
  - Оптимизирован поллинг флешки: добавлена привязка к Lifecycle (`LifecycleEventEffect`). Опрос порта USB аппаратно приостанавливается (`pausePolling()`), когда приложение уходит в фон, что исключает утечки ресурсов и снижает расход батареи.
  - Повышена отказоустойчивость: в `ArchiveSyncHelper` добавлены `finally` блоки, гарантирующие безопасный сброс визуальных состояний (`isArchiving`, `isRestoring`) при непредвиденных ошибках и отмене корутин.
  - Устранен баг мерцания плашки «Архив не подключен» на старых/медленных ОС (Android 12). Таймаут первичного ожидания монтирования флешки (`verifyConnectionAndWait`) увеличен до 20 секунд, а отображение плашек сделано строго взаимоисключающим.
